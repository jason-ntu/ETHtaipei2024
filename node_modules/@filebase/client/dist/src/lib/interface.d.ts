 ...
 */
export declare type Tagged<T, Tag> = T & {
    tag?: Tag;
};
...
export declare type FilesSource = Iterable<File> | Iterable<FileObject> | AsyncIterable<File> | AsyncIterable<FileObject>;
/**
 * CID in string representation
 */
export declare type CIDString = Tagged<string, CID>;
...
export interface CarStorerOptions {
    ...
    /**
     * Callback called after CAR has finished uploading.
     */
    onComplete?: () => void;
    /**
    ...
    maxRetries?: number;
    /**
     * Object Name to set for CAR on Pinning Service
    * */
    objectName?: string;
}
...
export declare type Deal = QueuedDeal | PendingDeal | FailedDeal | PublishedDeal | FinalizedDeal;
...
export declare type EncodedURL = Tagged<string, URL>;
...
export declare type StoreResponse<T extends TokenInput> = Result<EncodedError, EncodedToken<T>>;
/**
 ...
 * ```
 */
export declare type Encoded<T, Format extends Pattern<any, any>[]> = MatchRecord<T, Rule<Format[number]>>;
/**
 ...
 */
declare type Pattern<I, O> = ((input: I) => O) | [I, O];
export declare type MatchRecord<T, R extends Rule<any>> = {
    [K in keyof T]: MatchRule<T[K], R> extends never ? MatchRecord<T[K], R> : MatchRule<T[K], R>;
};
declare type MatchRule<T, R extends Rule<any>> = R extends (input: T) => infer O ? O : never;
declare type Rule<Format extends Pattern<any, any>> = Format extends [infer I, infer O] ? (input: I) => O : Format extends (input: infer I) => infer O ? (input: I) => O : never;
/**
